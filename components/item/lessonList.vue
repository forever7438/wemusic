	...
	<div class="lesson_content">
		<ul v-if="listInfo.length">
			<li v-for="(item, index) in listInfo" :key="index">
				...
				<image v-else :src="item.photo | imgformat"></image>
				<div class="lesson_messgae">
					...
					<p v-else class="lesson_winer">{{ item.teacher_name || item.name }}</p>
					<div class="lesson_pay">
						<span v-if="lessonType === 'lessonCopy' || lessonType === 'teacherDetail'" class="teacher_number">
							{{
								success
									? $t('index').teacher + '  &nbsp;&nbsp;&nbsp;' + $t('index').One_on_one
									: item.teacher_count > 0
									? $t('index').total + ' ' + item.teacher_count + ' ' + $t('index').teacherNum
									: $t('index').NoTeacher
							}}
						</span>
						<span v-else class="start_time">{{ item.start_time | timeDate }} {{ $t('index').Class_begins }}</span>
						<navigator hover-class="none" :url="'/pages/evaluate/evaluate?classId=' + item.class_id + '&orderId=' + item.order_id" v-if="item.type === '2'">
							<span class="go_pay">{{ $t('index').To_evaluate }}</span>
						</navigator>
						<navigator hover-class="none" :url="'/pages/lessonCopy/lessonCopy?musicId=' + item.id" v-if="item.type === '3'">
							<span class="go_pay">{{ $t('index').See }}</span>
						</navigator>
						<navigator hover-class="none" :url="'/pages/evaluate/evaluate?classId=' + item.class_id + '&orderId=' + item.order_id" v-if="item.type === '0'">
