			...
			<li v-for="(item, index) in listInfo" :key="index">
				...
				<div class="lesson_messgae">
					...
					<div class="lesson_pay">
						...
						<navigator hover-class="none" :url="'/pages/evaluate/evaluate?classId=' + item.class_id" v-if="lessonType === '-1'">
							<span class="go_pay">{{ $t('index').To_evaluate }}</span>
						</navigator>
						<navigator hover-class="none" :url="'/pages/evaluate/evaluate?classId=' + item.class_id" v-if="lessonType === '0'">
