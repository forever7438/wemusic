<template>
	<div>
		...
		<ul>
			<navigator hover-class="btn-hover" :url="'/pages/myCode/myCode?code=' + code">
				<li v-if="invite">{{ $t('index').viewInvitationCode }}</li>
			</navigator>
			<li class="go_index" @tap="goIndex">{{ $t('index').backHome }}</li>
		</ul>
	</div>
</template>
...
export default {
	...
	onLoad(obj) {
		...
		if (obj.wayNum == 0 || obj.invite == '-1' || obj.invite == 'undefined' || obj.invite == undefined) {
			this.invite = false;
		}
	},
	methods: {
